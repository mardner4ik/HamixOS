...
use crate::support::bit::{ReadBitsLtr, vlc::*};
use crate::support::{io, FloatExt, Lazy};
...
impl QuadsCodebook {
    #[inline(always)]
    pub fn read_quant<B: ReadBitsLtr>(&self, bs: &mut B) -> io::Result<(u8, u8, u8, u8)> {
...
impl PairsCodebook {
    #[inline(always)]
    pub fn read_dequant<B: ReadBitsLtr>(&self, bs: &mut B) -> io::Result<(f32, f32)> {
...
impl EscapeCodebook {
    #[inline(always)]
    pub fn read_quant<B: ReadBitsLtr>(&self, bs: &mut B) -> io::Result<(u16, u16)> {
        bs.read_codebook(&self.codebook).map(|(cw, _)| self.values[cw as usize])
    }
}
...
pub static ESC: Lazy<EscapeCodebook> = Lazy::new(|| make_value_codebook(&SPECTRUM_TABLES[10], escape_pair::<17>));
