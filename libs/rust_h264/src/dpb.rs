...
impl Dpb {
    ...
    /// FrameNumOffset for POC types 1 and 2 (spec 8.2.1.2, 8.2.1.3).
    pub fn frame_num_offset(&self, frame_num: u32, nal_unit_type: NalUnitType, max_frame_num: u32) -> u32 {
        if nal_unit_type == NalUnitType::SliceIdr {
            0
        } else if self.prev_frame_num > frame_num {
            self.prev_frame_num_offset.wrapping_add(max_frame_num)
        } else {
            self.prev_frame_num_offset
        }
    }

    /// Record the numbering state of the picture that was just decoded.
    pub fn end_picture(&mut self, frame_num: u32, frame_num_offset: u32, had_mmco5: bool) {
        if had_mmco5 {
    ...
    pub fn short_term_ref_list(&self, frame_num: u32, max_frame_num: u32) -> Vec<Rc<DecodedPicture>> {
        let mut refs: Vec<_> = self
            ...
            .map(|e| e.pic.clone())
            .collect();
        let wrap = |p: &DecodedPicture| -> i64 {
            if p.frame_num > frame_num {
                p.frame_num as i64 - max_frame_num as i64
            } else {
                p.frame_num as i64
            }
        };
        refs.sort_by(|a, b| wrap(b).cmp(&wrap(a)));
    ...
    pub fn apply_ref_list_modification(
        ...
        let pool: Vec<Rc<DecodedPicture>> = ref_list.clone();
        ...
        for &(idc, val) in ops {
            ...
            if idc == 2 {
                ...
                if let Some(found_pos) = pool.iter().position(|p| {
            ...
            if let Some(found) = pool.iter().find(|p| p.frame_num == pic_num) {
                let pic = found.clone();
    ...
    pub fn compute_poc(
        ...
        match sps.pic_order_cnt_type {
            ...
            1 => self.compute_poc_type1(sps, header, nal_unit_type, nal_ref_idc, frame_num_offset),
            2 => Self::compute_poc_type2(header.frame_num.wrapping_add(frame_num_offset), nal_unit_type, nal_ref_idc),
            _ => 0,
        }
    }
    ...
    fn compute_poc_type1(
        ...
        let abs_frame_num = if num_ref_frames_in_cycle > 0 {
            header.frame_num.wrapping_add(frame_num_offset) as i32
        } else {
            0
        };
...
mod tests {
    ...
    fn test_dpb_insert_and_clear() {
        ...
        assert_eq!(dpb.short_term_ref_list(u32::MAX, 16).len(), 2);
        dpb.clear();
        assert_eq!(dpb.short_term_ref_list(u32::MAX, 16).len(), 0);
    }
    ...
    fn test_sliding_window() {
        ...
        let refs = dpb.short_term_ref_list(u32::MAX, 16);
    ...
    fn test_unused_not_in_ref_list() {
        ...
        assert_eq!(dpb.short_term_ref_list(u32::MAX, 16).len(), 1);
    }
