...
pub struct Decoder {
    ...
    recycled: Vec<Rc<DecodedPicture>>,
    spare: Option<SpareMbBuffers>,
}

/// A decoded picture shared with the reference buffer, without copying planes.
///
/// Planes are stored at coded (macroblock-aligned) size; `stride_y` and
/// `stride_c` give the row step, `width`/`height` the visible size.
#[derive(Clone)]
pub struct SharedFrame {
    picture: Rc<DecodedPicture>,
    pub width: u32,
    pub height: u32,
    pub pic_order_cnt: i32,
}
...
impl SharedFrame {
    ...
    pub fn stride_c(&self) -> usize {
        self.picture.width as usize / 2
    }

    pub fn to_frame(&self) -> Frame {
        let (w, h) = (self.width as usize, self.height as usize);
        let (sy, sc) = (self.stride_y(), self.stride_c());
        let mut y = Vec::with_capacity(w * h);
        for r in 0..h {
            y.extend_from_slice(&self.picture.y[r * sy..r * sy + w]);
        }
        let (cw, ch) = (w / 2, h / 2);
        let mut u = Vec::with_capacity(cw * ch);
        let mut v = Vec::with_capacity(cw * ch);
        for r in 0..ch {
            u.extend_from_slice(&self.picture.u[r * sc..r * sc + cw]);
            v.extend_from_slice(&self.picture.v[r * sc..r * sc + cw]);
        }
        Frame { width: self.width, height: self.height, y, u, v, pic_order_cnt: self.pic_order_cnt }
    }
}

struct SpareMbBuffers {
    nc_luma: Vec<u8>,
    nc_cb: Vec<u8>,
    nc_cr: Vec<u8>,
    mvd_store: Vec<[i16; 2]>,
...
const RECYCLE_LIMIT: usize = 40;
const SPARE_PICTURES: usize = 3;
...
impl Decoder {
    ...
    pub fn decode_nal(&mut self, nal: &NalUnit) -> Result<Option<Frame>, DecodeError> {
        Ok(self.decode_nal_inner(nal)?.map(|f| f.to_frame()))
    }

    fn decode_nal_inner(&mut self, nal: &NalUnit) -> Result<Option<SharedFrame>, DecodeError> {
        match nal.nal_unit_type {
            ...
            NalUnitType::SliceIdr | NalUnitType::Slice => {
                ...
                let cabac = {
                    ...
                    let _ = peek.read_ue();
                    let _ = peek.read_ue();
                    peek.read_ue().ok().and_then(|id| self.pps_table.get(&id)).map(|p| p.entropy_coding_mode_flag).unwrap_or(false)
                };
                let pending_backup = if had_pending && !cabac {
                    self.pending.clone()
                } else {
                    None
                };
    ...
    pub fn flush(&mut self) -> Option<Frame> {
        self.finalize_pending().map(|f| f.to_frame())
    }

    /// Like [`decode_nal`](Self::decode_nal), but hands out the decoded
    /// picture without copying its planes.
    pub fn decode_nal_shared(&mut self, nal: &NalUnit) -> Result<Option<SharedFrame>, DecodeError> {
        self.decode_nal_inner(nal)
    }

    /// Like [`flush`](Self::flush), without copying planes.
    pub fn flush_shared(&mut self) -> Option<SharedFrame> {
        self.finalize_pending()
    }

    fn take_recycled(recycled: &mut Vec<Rc<DecodedPicture>>, luma: usize, chroma: usize) -> Option<DecodedPicture> {
        let index = recycled.iter().position(|p| Rc::strong_count(p) == 1 && p.y.len() == luma && p.u.len() == chroma)?;
        Rc::try_unwrap(recycled.swap_remove(index)).ok()
    }

    fn remember(&mut self, picture: &Rc<DecodedPicture>) {
        self.recycled.retain(|p| Rc::strong_count(p) > 1 || p.y.len() == picture.y.len());
        let mut idle = 0;
        self.recycled.retain(|p| {
        ...
        self.recycled.push(picture.clone());
    }
    ...
    fn finalize_pending(&mut self) -> Option<SharedFrame> {
        ...
        for &(op, param) in &ps.mmco_ops {
            match op {
                1 => {
                    let pic_num_to_remove = (ps.frame_num as i64 - ((param & 0xFFFF) as i64 + 1)).rem_euclid(ps.max_frame_num.max(1) as i64);
                ...
                3 => {
                    ...
                    let pic_num = (ps.frame_num as i64 - (abs_diff_minus1 as i64 + 1)).rem_euclid(ps.max_frame_num.max(1) as i64);
                    self.dpb
        ...
        let display_w = ps.frame.width;
        let display_h = ps.frame.height;
        let coded_w = (ps.mb_width * 16) as usize;
        if coded_w == 0 || display_w as usize > coded_w || (display_h as usize) * coded_w > ps.frame.y.len() {
            return None;
        }
        self.spare = Some(SpareMbBuffers {
            nc_luma: core::mem::take(&mut ps.nc_luma),
            nc_cb: core::mem::take(&mut ps.nc_cb),
            nc_cr: core::mem::take(&mut ps.nc_cr),
            mvd_store: core::mem::take(&mut ps.mvd_store),
            mvd_store_l1: core::mem::take(&mut ps.mvd_store_l1),
            mb_info: core::mem::take(&mut ps.mb_info),
            i4x4_modes: core::mem::take(&mut ps.i4x4_modes),
            mb_cbp: core::mem::take(&mut ps.mb_cbp),
            mb_chroma_pred: core::mem::take(&mut ps.mb_chroma_pred),
            mb_is_8x8dct: core::mem::take(&mut ps.mb_is_8x8dct),
            mb_skip: core::mem::take(&mut ps.mb_skip),
            mb_is_direct: core::mem::take(&mut ps.mb_is_direct),
            blk_is_direct: core::mem::take(&mut ps.blk_is_direct),
            is_i16x16: core::mem::take(&mut ps.is_i16x16),
            mb_slice_id: core::mem::take(&mut ps.mb_slice_id),
            mb_field_decoding: core::mem::take(&mut ps.mb_field_decoding),
        });

        let pic = Rc::new(DecodedPicture {
            y: core::mem::take(&mut ps.frame.y),
            u: core::mem::take(&mut ps.frame.u),
            v: core::mem::take(&mut ps.frame.v),
        ...
        self.remember(&pic);
        let shared = SharedFrame { picture: pic.clone(), width: display_w, height: display_h, pic_order_cnt: ps.poc };
        ...
        self.dpb.end_picture(ps.frame_num, ps.frame_num_offset, has_mmco5);
    ...
    fn decode_slice(&mut self, nal: &NalUnit) -> Result<(), DecodeError> {
        ...
        let pps = pps_id
            .and_then(|id| self.pps_table.get(&id))
            .or_else(|| self.pps_table.values().next())
        ...
        let frame_num_offset = self.dpb.frame_num_offset(header.frame_num, nal.nal_unit_type, max_pic_num);
        let current_poc = self
            .dpb
            .compute_poc(sps, &header, nal.nal_unit_type, nal.nal_ref_idc, frame_num_offset);

        // Build reference picture lists
        let mut ref_pic_list = if is_p_slice {
            let mut refs = self.dpb.short_term_ref_list(header.frame_num, max_pic_num);
        ...
        let ps = if is_continuation {
            ...
            let luma_len = (coded_width * coded_height) as usize;
            let chroma_len = luma_len / 4;
            let (y, u, v, mut mv_store_l0, mut mv_store_l1, mut ref_idx_store_l0, mut ref_poc_store_l0, mut ref_idx_store_l1) = match Self::take_recycled(&mut self.recycled, luma_len, chroma_len) {
                Some(old) => (old.y, old.u, old.v, old.mv_l0, old.mv_l1, old.ref_idx_l0, old.ref_poc_l0, old.ref_idx_l1),
                None => (vec![0u8; luma_len], vec![128u8; chroma_len], vec![128u8; chroma_len], Vec::new(), Vec::new(), Vec::new(), Vec::new(), Vec::new()),
            };
            refill(&mut mv_store_l0, total_mbs * 16, [0i16; 2]);
            refill(&mut mv_store_l1, total_mbs * 16, [0i16; 2]);
            refill(&mut ref_idx_store_l0, total_mbs * 16, -1i8);
            refill(&mut ref_poc_store_l0, total_mbs * 16, -1i32);
            refill(&mut ref_idx_store_l1, total_mbs * 16, -1i8);
            let mut spare = self.spare.take().unwrap_or(SpareMbBuffers {
                nc_luma: Vec::new(),
                nc_cb: Vec::new(),
                nc_cr: Vec::new(),
                mvd_store: Vec::new(),
            ...
            refill(&mut spare.nc_luma, total_mbs * 16, 0u8);
            refill(&mut spare.nc_cb, total_mbs * 4, 0u8);
            refill(&mut spare.nc_cr, total_mbs * 4, 0u8);
            refill(&mut spare.mvd_store, total_mbs * 16, [0i16; 2]);
            refill(&mut spare.mvd_store_l1, total_mbs * 16, [0i16; 2]);
            refill(&mut spare.mb_info, total_mbs, deblock::MbInfo::default());
            refill(&mut spare.i4x4_modes, total_mbs * 16, 2u8);
            refill(&mut spare.mb_cbp, total_mbs, 0u16);
            refill(&mut spare.mb_chroma_pred, total_mbs, 0u8);
            refill(&mut spare.mb_is_8x8dct, total_mbs, false);
            refill(&mut spare.mb_skip, total_mbs, false);
            refill(&mut spare.mb_is_direct, total_mbs, false);
            refill(&mut spare.blk_is_direct, total_mbs * 16, false);
            refill(&mut spare.is_i16x16, total_mbs, false);
            refill(&mut spare.mb_slice_id, total_mbs, 0u16);
            refill(&mut spare.mb_field_decoding, total_mbs.div_ceil(2), false);
            PictureState {
                frame: Frame {
                    width,
                    height,
                ...
                ref_poc_store_l0,
                ref_idx_store_l1,
                mvd_store: spare.mvd_store,
                mvd_store_l1: spare.mvd_store_l1,
                mb_info: spare.mb_info,
                i4x4_modes: spare.i4x4_modes,
                mb_cbp: spare.mb_cbp,
                mb_chroma_pred: spare.mb_chroma_pred,
                mb_is_8x8dct: spare.mb_is_8x8dct,
                mb_skip: spare.mb_skip,
                mb_is_direct: spare.mb_is_direct,
                blk_is_direct: spare.blk_is_direct,
                is_i16x16: spare.is_i16x16,
                mb_slice_id: spare.mb_slice_id,
                current_slice_id: 0,
                prev_mb_qp: slice_qp,
                ...
                mb_width,
                mb_height,
                mb_field_decoding: spare.mb_field_decoding,
