...
fn luma_mc_inner(
    ...
    if (1..=16).contains(&w) {
        ...
        let (first, second) = match (frac_x, frac_y) {
            (0, 0) => (Tap::Int(0, 0), None),
            (2, 0) => (Tap::H(0), None),
            (0, 2) => (Tap::V(0), None),
            (2, 2) => (Tap::Hv, None),
            (1, 0) => (Tap::Int(0, 0), Some(Tap::H(0))),
            (3, 0) => (Tap::H(0), Some(Tap::Int(1, 0))),
            (0, 1) => (Tap::Int(0, 0), Some(Tap::V(0))),
            (0, 3) => (Tap::V(0), Some(Tap::Int(0, 1))),
            (2, 1) => (Tap::H(0), Some(Tap::Hv)),
            (2, 3) => (Tap::Hv, Some(Tap::H(1))),
            (1, 2) => (Tap::V(0), Some(Tap::Hv)),
            (3, 2) => (Tap::Hv, Some(Tap::V(1))),
            (1, 1) => (Tap::H(0), Some(Tap::V(0))),
            (3, 1) => (Tap::H(0), Some(Tap::V(1))),
            (1, 3) => (Tap::V(0), Some(Tap::H(1))),
            _ => (Tap::V(1), Some(Tap::H(1))),
        };
        let needs = |tap: Tap| -> Option<(usize, usize)> {
            match tap {
                Tap::Hv => Some((0, h + 5)),
                Tap::H(dy) => {
                    let base = (2 + dy) as usize;
                    Some((base, base + h))
                }
                _ => None,
            }
        };
        let span = [Some(first), second].into_iter().flatten().filter_map(needs).fold(None, |acc: Option<(usize, usize)>, r| match acc {
            Some((lo, hi)) => Some((lo.min(r.0), hi.max(r.1))),
            None => Some(r),
        });
        let mut plane = [0i16; 21 * 16];
        if let Some((lo, hi)) = span {
            for j in lo..hi {
                let source = row(j as isize - 2, -2, w + 5);
                crate::x86::fir6_horizontal_i16(source, &mut plane[(j - lo) * w..(j - lo + 1) * w], w);
            }
        }
        let base = span.map(|(lo, _)| lo).unwrap_or(0);
        let mut a = [0u8; 16];
        let mut b = [0u8; 16];
        for r in 0..h {
            let mut fill = |tap: Tap, dst: &mut [u8]| match tap {
                Tap::Int(dx, dy) => dst[..w].copy_from_slice(row(r as isize + dy, dx, w)),
                Tap::H(dy) => {
                    let j = (2 + dy) as usize + r - base;
                    crate::x86::round_i16(&plane[j * w..(j + 1) * w], dst, w);
                }
                Tap::V(dx) => crate::x86::half_pel_v(vrows(dx, r as isize, w), dst, w),
                Tap::Hv => {
                    let j = r - base;
                    let rows6: [&[i16]; 6] = core::array::from_fn(|k| &plane[(j + k) * w..(j + k + 1) * w]);
                    crate::x86::fir6_vertical_i16(rows6, dst, w);
                }
            };
            let out = &mut output[r * w..(r + 1) * w];
            match second {
                None => fill(first, out),
                Some(tap) => {
                    fill(first, &mut a);
                    fill(tap, &mut b);
                    crate::x86::average(&a, &b, out, w);
                }
            }
        }
        return;
...
pub fn chroma_mc(
    ...
    if in_bounds {
        ...
        #[cfg(target_arch = "x86_64")]
        {
            crate::x86::chroma_bilinear(ref_plane, top_off, ref_width, block_w, block_h, output, c00 as u16, c01 as u16, c10 as u16, c11 as u16);
        }
...
pub fn bi_pred_avg(pred_l0: &[u8], pred_l1: &[u8], output: &mut [u8]) {
    let n = output.len().min(pred_l0.len()).min(pred_l1.len());
    #[cfg(target_arch = "x86_64")]
    crate::x86::average(pred_l0, pred_l1, output, n);
    #[cfg(not(target_arch = "x86_64"))]
    {
        let (output, pred_l0, pred_l1) = (&mut output[..n], &pred_l0[..n], &pred_l1[..n]);
        for i in 0..n {
...
pub fn weighted_uni(pred: &mut [u8], log2_denom: u32, weight: i32, offset: i32) {
    if log2_denom <= 7 && weight == (1 << log2_denom) && offset == 0 {
        return;
    }
    if (-128..=127).contains(&weight) && (-128..=127).contains(&offset) && log2_denom <= 7 {
        let w = weight as i16;
        let o = offset as i16;
        let shift = log2_denom as i16;
        let round: i16 = if log2_denom == 0 { 0 } else { 1 << (log2_denom - 1) };
        for p in pred.iter_mut() {
            let v = ((*p as i16 * w + round) >> shift) + o;
...
pub fn weighted_bi_implicit(pred_l0: &[u8], pred_l1: &[u8], output: &mut [u8], w0: i32, w1: i32) {
    if w0 == 32 && w1 == 32 {
        bi_pred_avg(pred_l0, pred_l1, output);
        return;
    }
    let n = output.len().min(pred_l0.len()).min(pred_l1.len());
    let (output, pred_l0, pred_l1) = (&mut output[..n], &pred_l0[..n], &pred_l1[..n]);
    if (-64..=128).contains(&w0) && (-64..=128).contains(&w1) {
        let (a16, b16) = (w0 as i16, w1 as i16);
        for i in 0..n {
            let v = (pred_l0[i] as i16 * a16 + pred_l1[i] as i16 * b16 + 32) >> 6;
            output[i] = v.clamp(0, 255) as u8;
        }
        return;
    }
    let round = 1i32 << 5;
    for i in 0..n {
        output[i] = ((pred_l0[i] as i32 * w0 + pred_l1[i] as i32 * w1 + round) >> 6).clamp(0, 255) as u8;
    }
}
